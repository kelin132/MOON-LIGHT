...
export default async function ShopPage() {
  await dbConnect();
  const cards = await Card.find({}).sort({ price: 1 }).limit(60).lean();

  return (
    <div className="space-y-10">
      <div>
        <h1 className="font-display text-3xl">Shop</h1>
        <p className="text-text-lo mt-1">Spend your coins on cards and Haven goods.</p>
      </div>

      <section id="cards" className="scroll-mt-6 space-y-4">
        <h2 className="font-display text-2xl">Cards</h2>
        {cards.length === 0 ? (
          <div className="text-text-lo">No cards are listed yet.</div>
        ) : (
              ...
              <Link key={String(card._id)} href={`/cards/${card._id}`}>
                <GlassCard className="!p-0 overflow-hidden h-full flex flex-col">
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="mt-auto pt-3 text-moongold font-mono text-sm">
                      {card.price ?? 0} coins
                    </div>
                  </div>
                </GlassCard>
